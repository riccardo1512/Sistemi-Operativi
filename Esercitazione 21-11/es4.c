...
typedef struct node
{
	char free; // usa 3 byte di meno rispetto all'int
	size_t size;
	struct node *next;
} node;

node *first; // variabile globale, evita di dovermela passare ogni volta
...
node *search_free_node(size_t size)
{
	...
	{ // cerchiamo un nodo libero di dimensione sufficiente, devo considerare che dovrò creare l'altro blocco
		cur = cur->next;
	}
...
void *alloc_in_free_node(node *free_node, size_t size)
{
	...
	{
		node *new_node = (node *)((void *)free_node + sizeof(node) + size); // serve il cast a void* perchè in questo modo +1 è uguale a +1byte, altrimenti con +1 andrebbe avanti di dimensione del nodo byte
		new_node->free = 1;
